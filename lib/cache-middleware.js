  ...
  return function cacheMiddleware(req, res, next) {
    ...
    if (cache.has(cacheKey)) {
      ...
      cache.getLater(cacheKey, function(result) {
        ...
        if (result === null) {
          cache.logMessage(`retry {${cacheKey}}`);
      ...
      cache.logMessage(`locked {${cacheKey}}`);
      cache.lock(cacheKey);

      let getBody = responseBody.from(res);

      req.on('aborted', () => {
        cache.logMessage(`abandoned {${cacheKey}}`);
      ...
      res.on('finish', () => {
        ...
        if (res.statusCode < 400) {
          cache.logMessage(`set {${cacheKey}} expires in ${expires}ms`);
          cache.set(cacheKey, result, expires);
        } else {
          cache.logMessage(`http error ${res.statusCode} on ${cacheKey}, not caching`);
        }

        cache.unlock(cacheKey, result);
        cache.logMessage(`unlocked {${cacheKey}}`);
      });
