...
function hasInvalidCharacters(key, value) {
  return (typeof(key) === 'string' && !!key.match(invalidCharacters)) || (typeof(value) === 'string' && !!value.match(invalidCharacters))
}
  ...
  proxy.on('proxyRes', function(proxyRes, req) {
    if (proxyRes && proxyRes.headers) {
      ...
      Object.entries(proxyRes.headers).forEach(([key,value]) => {
        if (hasInvalidCharacters(key, value)) {
          log({
            proxyError: "Invalid header in proxy response",
            url:        req.url,
            detail:     `${key} => ${value}`
          });
